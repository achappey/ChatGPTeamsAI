...
namespace ChatGPTeamsAI.Data.Clients.Microsoft
{
    internal partial class GraphFunctionsClient
    {
        ...
        [MethodDescription("Planner", "Search for all user Planners, optionally filtered by a search term", "ExportPlanners")]
        public async Task<ChatGPTeamsAIClientResponse?> SearchPlanners(
            ...
            [ParameterDescription("The number of items to skip")] string? skip = null)
        {
            ...
            var skipItems = !string.IsNullOrEmpty(skip) ? int.Parse(skip) : 0;

            var items = planners.Skip(skipItems).Take(PAGESIZE).Select(_mapper.Map<Models.Microsoft.PlannerPlan>);

            return ToChatGPTeamsAIResponse(items, null, (skipItems + PAGESIZE).ToString());
        }

        [MethodDescription("Export", "Export a list of Planners")]
        public async Task<ChatGPTeamsAIClientResponse?> ExportPlanners(
           [ParameterDescription("The search term to filter planners by title (optional).")] string? searchTerm = null)
        {
            var filterOptions = new List<QueryOption>();
